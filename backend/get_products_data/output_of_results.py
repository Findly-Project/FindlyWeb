...
async def output_of_results(query: str) -> MarketPlaceList:
    ...
    for func_name, func in get_data_functions.items():
        ...
        regular_expression_filter: List[str] = (
            filter_regular_expression.regular_expression(query, product_names)
        )
        best_items: ProductList = SortProductList.sort_by_name(
            regular_expression_filter, pars_data
        )
        best_items_sorted_by_price: ProductList = SortProductList.sort_by_price(
            best_items
        )

        result_data: ProductList = filter_for_category_based_on_price.filter_by_price(
            best_items_sorted_by_price
        )
